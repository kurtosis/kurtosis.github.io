...
3. A [value head][code-value], which takes the embedding produced by the torso and generates a distribution of expected returns.
...
So far we've described our network architecture and a method of training it on synthetic demonstrations. But how do we actually play TensorGame and search for a low-rank decomposition of $$\mathcal{T}$$? AlphaTensor uses MCTS, as described in the [AlphaZero][alphazero] and [Sampled MuZero][muzero] papers. MCTS uses the output of the network's policy and value heads, along with an upper-confidence bound decision rule to explore the game tree. The implementation of MCTS ([code][code-actor-pred]) involves a fairly deep call stack, with several nested loops and can be difficult to follow from the code alone. We'll build some intuition by illustrating this graphically.
...
DeepMind's MCTS procedure uses $$n_{samples}=32$$ and $$n_{sim}=800$$, producing trees with up to 25,600 nodes.
