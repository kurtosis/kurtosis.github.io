...
- The optimal temperature $$T^*$$ (which maximizes $$\mathbb{E}[\text{pass@k}]$$) depends on $$k$$. I present results from a toy model showing that $$T^*(k)$$ follows a power law over several decades of $$k$$.
